...
* 比如spring的bean的后置处理器，就是一些模版。可以利用此模式来设计一些扩展性比较高的组件。

## 观察者模式

* spring的事件驱动模型，就是典型的观察者设计模式，因为平时用的比较多，这里也不做过多总结了。

## 单例模式

* spring的上下文对象就是一个单例对象。每次获取时，如果已经创建过，则使用之前创建的。

## 代理模式

* spring的aop平时用的也比较多，这里也不做过多总结。
